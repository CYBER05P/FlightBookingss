  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-100 to-blue-200 p-4 md:p-8">
      ...
      <div className="flex flex-wrap justify-center gap-4 mb-4">
        <input type="text" placeholder="From City" value={fromFilter} onChange={(e) => setFromFilter(e.target.value)} className="border px-4 py-2 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" />
        <input type="text" placeholder="To City" value={toFilter} onChange={(e) => setToFilter(e.target.value)} className="border px-4 py-2 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" />
        <select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)} className="border px-4 py-2 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
          <option value="ALL">All Status</option>
          <option value="ACTIVE">Active</option>
          <option value="CANCELLED">Cancelled</option>
        </select>
        <select value={sortOption} onChange={(e) => setSortOption(e.target.value)} className="border px-4 py-2 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
          <option value="price">Price (Lowest First)</option>
          <option value="time">Departure Time (Earliest First)</option>
        </select>
        <button onClick={handleReset} className="flex items-center gap-2 bg-red-500 hover:bg-red-600 text-white px-4 py-2 rounded-lg transition">
          <FaSync /> Reset
        </button>
      </div>

      <p className="text-center text-gray-700 mb-6">{sortedFlights.length} flight{sortedFlights.length !== 1 && 's'} found</p>
      ...
      {sortedFlights.length === 0 ? (
        ...
        <div className="grid gap-8 max-w-5xl mx-auto">
          {sortedFlights.map((flight, index) => (
            <motion.div key={index} className="bg-white p-6 rounded-2xl shadow-xl flex flex-col md:flex-row justify-between items-center border hover:scale-[1.02] hover:shadow-2xl transition-transform" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.1 }}>
              ...
              <div className="mt-6 md:mt-0 text-right space-y-3">
                ...
                <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.98 }} onClick={() => handleBook(flight)} disabled={flight.flightStatus !== 'ACTIVE'} className={`mt-4 px-6 py-2 rounded-full text-white font-semibold transition ${flight.flightStatus === 'ACTIVE' ? 'bg-gradient-to-r from-blue-500 to-blue-700 hover:from-blue-600' : 'bg-gray-400 cursor-not-allowed'}`}>
